  ...
  try {
    ...
    if (page) {
      ...
      const countResult = await query(
        'SELECT COUNT(*) as total FROM expenses WHERE vehicle_id = $1',
...
export async function POST(
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO expenses (vehicle_id, amount, description, category, date, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, 'PENDING', NOW(), NOW())
       RETURNING *`,
      [(await params).vehicleId, amount, description, category, date]
    );

    return createSuccessResponse(result.rows[0], 201);
  } catch (error) {
    console.error('Error creating expense:', error);
    return createErrorResponse('Erreur lors de la création de la dépense', 500);
  }
}
